...
Date: 2026-04-17
Repo: `/Users/thinh/projects/.workspaces/PRO-226/_space`

## Current Status

- 2026-04-23 board home chat is now wired to the real streaming agent API. `apps/web/src/components/BoardView.tsx` replaces the echo stub with `fetchSimpleHistory()` history loads, `streamMessage()` live assistant text, session-key reuse via `getSessionKey()`, `/abort` stop handling via `postAbort()`, selected-agent resets, and near-bottom-only autoscroll. Docs updated in `README.md`, `docs/llms.md`, and `docs/handoff/board-extension.md`. Verification pending in current worktree: `pnpm --filter @aihub/web exec tsc --noEmit`.
- 2026-04-23 `apps/web/src/components/BoardChatRenderer.tsx` added on `board`: standalone board full-history renderer with exported `BoardLogItem`, `buildBoardLogs(messages)`, and `BoardChatLog`. It inlines assistant/user text, thinking blocks, tool calls, and diff rendering from `FullHistoryMessage[]` without modifying `BoardView.tsx`. Verification passed: `pnpm --filter @aihub/web exec tsc --noEmit`.
- 2026-04-23 board home chat now supports full send/stop + queued follow-up flow. `apps/web/src/components/BoardView.tsx` keeps the textarea editable while streaming, splits Send and Stop into separate buttons, queues follow-up user messages locally when a run is active, flushes them sequentially after the active run finishes, and waits for `/abort` to settle before resuming any remaining queue. Docs updated in `README.md` and `docs/llms.md`. Verification pending in current worktree: `pnpm --filter @aihub/web exec tsc --noEmit`.
- 2026-04-23 board home chat now uses full history + tool-call rendering. `apps/web/src/components/BoardView.tsx` switched from simple history to `fetchFullHistory()`, renders persisted history through `BoardChatLog`, streams live assistant text plus tool call/result rows for foreground and background runs, keeps queued follow-up user messages separate until they start running, and preserves partial live text on abort before resuming any remaining queue. Docs updated in `README.md` and `docs/llms.md`. Verification passed: `pnpm --filter @aihub/web exec tsc --noEmit`.
- 2026-04-23 ChatView now supports drag-and-drop file attach across the message history, composer, and `+` button with zone-specific visual feedback. `apps/web/src/components/ChatView.tsx` moved drop handling to root capture listeners so nested controls accept file drops through the same pending-upload path, and `apps/web/src/components/ChatView.test.tsx` covers history/input/button drops plus composer hover feedback. Verification passed: `pnpm exec vitest run apps/web/src/components/ChatView.test.tsx`, `pnpm exec tsc -b`, `pnpm exec eslint apps/web/src/components/ChatView.tsx apps/web/src/components/ChatView.test.tsx`.
- 2026-04-23 ChatView abort preservation fix landed. `apps/web/src/components/ChatView.tsx` no longer closes its live websocket or clears streaming buffers immediately on Stop; instead it waits for the interrupted run to finish, preserves any partial assistant text/tool/thinking blocks already streamed, then shows `Interrupted`. Gateway `apps/gateway/src/agents/runner.ts` now includes `meta.aborted` on normal run completion events so `/abort` and Stop share the same client handling. Verification passed: `pnpm exec vitest run apps/web/src/components/ChatView.test.tsx`, `pnpm exec tsc -b`.
- 2026-04-23 repo root now has `scripts/create-local-config.js` for local bootstrap, exposed as `pnpm init-dev-config`. It scans `3001-3100` for the first free UI port and `4001-4100` for the first free gateway port, fills `scripts/config-template.json`, and writes `./.aihub/aihub.json` relative to the repo root. Verification passed: `pnpm exec eslint scripts/create-local-config.js`, `pnpm init-dev-config` (generated `./.aihub/aihub.json` with `ui.port=3001`, `gateway.port=4001`, `apiUrl=http://127.0.0.1:4001/aihub` in this worktree).
- 2026-04-22 staging gateway bootstrap on `agentic-workflows` needed two startup fixes: `container/agent-runner/Dockerfile` no longer runs `chmod -R a+rX /app` because that recursive chmod over the installed tree delayed first-run `aihub-agent:latest` image builds enough to block binding `3003/4003`, and `apps/web/vite.config.ts` now derives `server.allowedHosts` and `preview.allowedHosts` from configured `server.baseUrl` / `web.baseUrl` hostnames so Vite preview accepts `hub.cloudi-fi.algodyn.ai` in staging.
- 2026-04-22 multi-user login startup race fixed. Gateway startup now initializes extensions before opening the HTTP listener, which prevents `/api/auth/*` from throwing `multi-user runtime not initialized` during early login/session requests. Regression coverage added in `apps/gateway/src/cli/gateway.test.ts`. Verification passed: `pnpm exec vitest run apps/gateway/src/cli/gateway.test.ts`, `pnpm exec tsc -b`.
- 2026-04-21 PRO-200 webhook verification semantics corrected in current worktree: removed hard-coded GET `?challenge=` shortcuts, but kept `verification` field-based. Requests short-circuit only when the configured header/payload field is present; otherwise they proceed through normal webhook execution. Verification pending in current worktree: focused webhook tests.
- 2026-04-21 startup sequencing fix landed in `space/PRO-200`: extension load order now prioritizes `webhooks` ahead of Slack/Discord so `/hooks` runtime and URL logging are not blocked behind Socket Mode handshakes, and Slack bots now use a `SocketModeReceiver` with `clientPingTimeout: 20000` to avoid premature 5s pong disconnect warnings during slower connects. Verification pending in current worktree: focused gateway/slack tests plus manual `AIHUB_HOME=... pnpm dev` repro.
- 2026-04-20 PRO-200 Webhooks Extension generic verification config added. Per-webhook `verification: { location: "header"|"payload", fieldName }` now short-circuits setup requests containing the configured header or JSON payload key before signature checks or agent invocation, returning `{ ok: true, verification: true }` and logging headers/payload. Docs updated with Notion `verification_token` example. Verification passed: `pnpm exec vitest run packages/extensions/webhooks/`, `pnpm build`, `pnpm lint` (0 errors, existing Pi SDK warnings).
- 2026-04-20 PRO-200 Webhooks Extension review fixes complete. Routes now validate secrets through an mtime-cached `$AIHUB_HOME/webhook-secrets.json` read so CLI rotations take effect without restart; secret writers chmod files to `0600`; request bodies are streamed with a byte cap; prompt files resolving outside the agent workspace are rejected; async webhook failures emit traceable `agent.stream` error events with webhook metadata. Verification passed: `pnpm exec vitest run packages/extensions/webhooks/`, `pnpm test:cli`, `pnpm build`, `pnpm lint` (0 errors, existing Pi SDK warnings).
- 2026-04-20 PRO-200 Webhooks Extension Tasks 4-6 complete. Added per-run trace context for Langfuse (`enabled`, explicit surface/name, metadata), webhook traces as `aihub:webhook:<agentId>` with request metadata, `langfuseTracing: false` suppression, HMAC-SHA256 verification for GitHub/Notion/Zendesk with `$env:` signing secrets, and Notion/Zendesk GET `challenge` responses that do not invoke agents. Verification passed: focused webhooks/langfuse tests, `pnpm test:shared`, `pnpm test:gateway`, `pnpm lint`, `pnpm build`.
- 2026-04-20 PRO-200 Webhooks Extension Tasks 1-3 complete. Added `@aihub/extension-webhooks`, agent-level `webhooks` config schema, auto activation from per-agent config, `/hooks/:agentId/:name/:secret` forwarding outside `/api`, `$AIHUB_HOME/webhook-secrets.json` generation, startup URL logging, inline/file prompt resolution, `$WEBHOOK_*` interpolation, and fresh `webhook:<agentId>:<name>:<requestId>` sessions. Verification passed: `pnpm test:shared`, `pnpm test:gateway`, focused webhooks tests, `pnpm lint`, `pnpm build`.
- 2026-04-20 PRO-200 Webhooks Extension Tasks 7-9 complete. Added example prompt templates for Notion/Zendesk/GitHub, `aihub webhooks rotate <agentId> <webhookName>`, and per-webhook `maxPayloadSize` enforcement with 413 responses.
- PRO-224 adds Langfuse history-event tracing for user input, model usage, tool spans, and guarded flush/shutdown warnings.
- PRO-226 Slack Channel Support: fully implemented and merged. Added Slack Bolt Socket Mode runtime with channel/DM routing, per-agent bot tokens (multiple bots per workspace), mention/user gates, thread policies (channel + DM), reaction forwarding, `/new`/`/stop`/`/help`/`/ping` slash commands, 4000-char mrkdwn chunking, optional live thinking thread replies, thinking reactions, Slack mrkdwn conversion, broadcasts, shared config types, and 661 gateway tests. README updated with Channels section documenting Discord and Slack app manifest setup. Per-agent `agent.slack` config creates dedicated bots with own token/appToken pairs; component `components.slack` creates shared routed bot. Registry auto-detects per-agent slack and loads component accordingly.
...
- 2026-04-17 PRO-226 Slack Channel Support complete. Implemented Slack Bolt Socket Mode component with full feature parity alongside Discord: channel/DM routing, per-agent bot tokens (multiple bots per workspace), 5-stage message gating pipeline, thread policies per-channel and per-DM, reaction forwarding, slash commands (/new, /stop, /help, /ping), 4000-char mrkdwn chunking, thinking-reaction indicators, optional live thinking thread replies (`showThinking`, `deleteThinkingOnComplete`) with pending-post cleanup and forced `sessionId` override for tentative same-key binding, Slack mrkdwn conversion for direct replies and broadcasts, cross-source broadcasts, user allowlists enforced on both messages and commands. Shared types: `SlackComponentConfigSchema`, `SlackAgentConfigSchema`, `dm.threadPolicy`. Component registry auto-detects per-agent `agent.slack` and loads component even without `components.slack`. Dev mode keeps channels active. README updated with Channels section for Discord + Slack plus manifest setup. 79 test files, 661 tests, all green.
- 2026-04-15 PRO-210 chat stop/abort UX fixed for dockerized agents. Three issues addressed: (a) `apps/web/src/components/ChatView.tsx` send button now toggles into a red Stop button while `isStreaming()` and POSTs `/abort` via new `postAbort()` helper in `apps/web/src/api/client.ts`; (b) `/stop` is now a recognized abort trigger alongside `/abort` (`apps/gateway/src/sessions/store.ts#DEFAULT_ABORT_TRIGGERS`); (c) container `ipcDir` (`$AIHUB_HOME/ipc/<agentId>/input`) is wiped at the start of every `adapter.run` in `apps/gateway/src/sdk/container/adapter.ts` so leftover queued IPC messages from a prior container can no longer be picked up by a fresh one — fixes the "old `/stop` replayed in the new session after `/new`" bug. Verified in Chrome on `http://127.0.0.1:3001/chat/sally` (Send→red Stop toggle, click Stop aborts, `/stop` triggers `Received close IPC sentinel`, `/new` then `hey` returns a fresh greeting). `pnpm test` 781/781 green.
- 2026-04-15 PRO-210 connector-tool RPC path fixed end-to-end. Container tool calls were hitting the stale `http://gateway:4000` (unreachable on the internal-only `aihub-agents` network) and, after switching to `host.docker.internal`, a hard-coded `4000` that did not match the dev gateway's actual listen port. Fixes: (a) `apps/gateway/src/sdk/container/adapter.ts` `resolveContainerGatewayUrl()` now prefers `config.server.baseUrl`, then `process.env.AIHUB_GATEWAY_PORT`, then `config.gateway.port`, then `4000`, returning `http://host.docker.internal:<port>`; (b) `apps/gateway/src/server/index.ts` now sets `process.env.AIHUB_GATEWAY_PORT = resolvedPort` at listen time so the adapter always sees the real port (including the dev-mode auto-selected one); (c) `scripts/dev.ts` forwards `AIHUB_GATEWAY_PORT` into the gateway child env for robustness; (d) `container/agent-runner/src/runner.ts#callConnectorTool` now surfaces the gateway body on non-OK responses for diagnosable errors. Verified with `cloudifi_admin.list_companies` — returns `200` with 206 companies.
- 2026-04-15 PRO-210 onecli sandbox proxy wiring fixed: agent container sandbox network is `internal:true`, so `host.docker.internal` was unreachable and every model call failed with `Connection error`. Fixed by (a) adding `onecli.sandbox.{network,url}` to `OnecliConfigSchema`, (b) attaching the agent container to that extra docker network post-spawn via `docker network connect` in `apps/gateway/src/sdk/container/adapter.ts`, (c) pointing the container proxy URL at `http://onecli:10255` (DNS inside `config_onecli`), (d) switching the agent container env from `ANTHROPIC_BASE_URL`/`OPENAI_BASE_URL` to `HTTP_PROXY`/`HTTPS_PROXY`/`NO_PROXY` (pi-ai ignores the base-URL envs — it reads `baseUrl` from `models.json`), and (e) installing a global undici `EnvHttpProxyAgent` (with onecli CA) in `container/agent-runner/src/http-client.ts` so pi-ai's `fetch` goes through onecli. Sally now replies successfully via the requesty provider through the proxy.
