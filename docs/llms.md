...
- Project UI live refresh is event-driven via `/ws` broadcasts: kanban/project overview refetches on project `file_changed` events and subagent lifecycle `subagent_changed` events, project detail refetches on project file changes (`README.md`/`SPECS.md`/`THREAD.md`), and project subagent panels refetch immediately on `agent_changed` with a 2s polling fallback to recover from missed websocket events. Board overview deliberately ignores `agent_changed` because running subagent stream logs can emit it several times per second.
...
- Projects orchestrator v0.2 is opt-in via `extensions.projects.orchestrator`. When enabled, the daemon lives inside `packages/extensions/projects`, polls at `poll_interval_ms`, dispatches configured status bindings, and currently supports `statuses.todo.profile` Worker spawns plus `statuses.review.profile` Reviewer spawns with `source: "orchestrator"`. Worker spawns lock `todo` projects to `in_progress`; Reviewer spawns leave projects in `review` and rely on active-run dedupe/cooldown, then prompt reviewers to move passing work to `ready_to_merge` or failing work back to `todo`. Manual subagent runs default to `source: "manual"` and do not count against orchestrator concurrency.
