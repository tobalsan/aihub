...
- `/board/projects/:projectId` is Board extension project detail route. It uses `BoardProjectDetailPage` (tabs: Pitch | Slices | Thread | Activity). **Post-refactor model: slices are kanban unit; projects track lifecycle only (`shaping → active → done / cancelled`).** Slices tab embeds `SliceKanbanWidget` (scoped to project). Legacy `ProjectsBoard`/`ProjectsOverview` replaced for board lifecycle surfaces.
