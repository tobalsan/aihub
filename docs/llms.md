...
- Admin-only extension catalog for the Edit-Agent hub: `GET /api/agents/:agentId/extensions` (staff-only in multi-user mode) returns every available extension — built-in static registry (`apps/gateway/src/extensions/registry.ts`) plus a runtime scan of `$AIHUB_HOME/extensions` (`discoverExternalExtensions`) — each with the agent's enabled state (`agent.extensions[id].enabled !== false`), config JSON-schema (from `Extension.configJsonSchema`, populated by `defineToolExtension` via `zod-to-json-schema`), `requiredSecrets`, and a config tier: `bespoke-route` (has `routePrefixes`) > `auto-form` (has a meaningful schema) > `toggle-only`. Discovery is accurate — unloadable built-ins are omitted (no ghosts), each id appears once. Builder + tier logic live in `apps/gateway/src/extensions/catalog.ts`; the Edit-Agent page lists them with a clickable on/off toggle.
