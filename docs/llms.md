...
- Projects orchestrator v0.1 is opt-in via `extensions.projects.orchestrator`. When enabled, the daemon lives inside `packages/extensions/projects`, polls at `poll_interval_ms`, selects active `todo` projects with no running orchestrator-sourced subagent, and spawns the configured `statuses.todo.profile` with `source: "orchestrator"`. Manual subagent runs default to `source: "manual"` and do not count against orchestrator concurrency.
