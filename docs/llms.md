...
- Project and slice detail `Agent` tabs use `AgentRunChatPanel`: a runtime subagent session sidebar plus board-style transcript chat. It lists individual run attempts newest-first, auto-selects the newest run with visible transcript content, supports `?tab=agent&run=<runId>` deep links, keeps archived runs in a collapsed bottom section, and exposes Stop/Archive/Delete plus queued resume messages without raw-log or refresh controls. Live refresh depends on `subscribeToSubagentChanges({ onSubagentChanged })`; the panel reloads run state/logs from HTTP when runtime subagent websocket pulses arrive.
