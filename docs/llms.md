...
- Startup flow:
  ...
  2. If enabled, it creates `$AIHUB_HOME/auth.db`, runs Better Auth migrations, creates the custom `agent_assignments`, `teams`, and `team_members` tables, and initializes the auth runtime before binding the HTTP listener
