...
- `/projects/:id?` is the Board extension's project detail route. The Board extension hosts `ProjectListGrouped` (home: projects by lifecycle status) and `ProjectDetailPage` (tabs: Pitch | Slices | Thread | Activity). **Post-refactor model: slices are the kanban unit; projects track lifecycle only (`shaping → active → done / cancelled`).** The Slices tab embeds `SliceKanbanWidget` (scoped to that project). Legacy `ProjectsBoard`/`ProjectsOverview` replaced by these components.
