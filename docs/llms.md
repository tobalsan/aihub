...
`packages/extensions/orchestrator` is registered as optional `extensions.orchestrator` with `/api/orchestrator` routes. Current implementation covers v1 scaffolding: Linear GraphQL client/tool boundary, workflow fallback/merge/render, repo/profile/concurrency/claims/retry pure modules, workspace layout (default `$AIHUB_HOME/workspaces`, override with `extensions.orchestrator.workspacesRoot`), SQLite state store, hook runner, export helper, health/workflow/runs/release/manual-claim/export/webhook stub routes, and default `$AIHUB_HOME/WORKFLOW.md` creation. `LINEAR_API_KEY` stays in gateway process; routes return disabled health/503 behavior when missing. Projects/board remain live until Phase 5 HITL cleanup.
