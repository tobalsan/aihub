...
- Project detail mirrors the slice Agent surface for shaping runs: the `Agent` tab reuses `SubagentRunsPanel` with a client-side filter for runs where `projectId` matches and `sliceId` is unset, the header/card show a single shaping run pill (`Running`/`Stalled`/`Error`), and the project detail Recent Runs strip lists recent project-level shaping runs.
