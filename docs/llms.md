...
- Projects home uses a simplified project kanban with columns `Triage`, `Shaping`, `Active`, `Ready to merge`, and `Done`; any number of columns can be expanded and the expanded set is persisted. The create form stores the initial idea/prompt in `README.md` (frontmatter + body) so shaping agents can turn it into `PITCH.md`. The header `Archive` action opens `/projects/archive`, which groups `.archive` projects and `cancelled` projects separately instead of rendering an archive section above the kanban.
...
- Project and slice detail `Agent` tabs use `AgentRunChatPanel`: a runtime subagent session sidebar plus board-style transcript chat. It lists individual run attempts newest-first, auto-selects the newest run with visible transcript content, supports `?tab=agent&run=<runId>` deep links, keeps archived runs in a collapsed bottom section, and exposes Stop/Archive/Delete plus queued resume messages without raw-log or refresh controls.
...
- Project shaping pipeline runs as a second orchestrator phase configured by `extensions.projects.orchestrator.shaping_statuses` (ordered `shaping:<stage>` keys with `profile`, `max_concurrent`, optional `stall_threshold_ms`). Profiles with `type: "shaper"` dispatch at project level (no `sliceId`) when a project's status matches the key; only one shaper runs per project. Prompts load from `.aihub/prompts/<ProfileName>.md` with `${variable}` substitution and fail on unresolved variables, falling back to a built-in stage prompt. Project moves stamp `last_status_change_at`; stale shaping stages receive an Orchestrator project THREAD comment and move to `shaping:blocked`. Board lifecycle mapping groups any `shaping:*` under Shaping and cards show the sub-status badge.
