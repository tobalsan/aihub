...
This slice deliberately ships without WORKFLOW.md, without repo/worktree support, without SQLite persistence, without HTTP routes beyond a stub, and without the dashboard. It exists to prove the runtime loop end-to-end against the real Linear API.
...
- [ ] `OrchestratorExtensionConfigSchema` added to `packages/shared/src/types.ts` and wired into `ExtensionsConfigSchema`; supports `teamKey`, `repos`, `defaultRepo`, `poll.intervalMs`, `poll.jitterMs`, and `notifyChannel`.
- [ ] `LinearClient` performs the documented poll query (`team.key.eq` + `state.name.in`) and supports `commentCreate` + `issueUpdate` mutations; missing `LINEAR_API_KEY` disables the extension and routes return 503.
