...
- 2026-04-17 PRO-226 Slack Channel Support complete. Implemented Slack Bolt Socket Mode component with full feature parity alongside Discord: channel/DM routing, per-agent bot tokens (multiple bots per workspace), 5-stage message gating pipeline, thread policies per-channel and per-DM, reaction forwarding, slash commands (/new, /stop, /help, /ping), 4000-char mrkdwn chunking, thinking-reaction indicators, optional live thinking thread replies (`showThinking`, `deleteThinkingOnComplete`) with pending-post cleanup and `sessionId` matching, Slack mrkdwn conversion for direct replies and broadcasts, cross-source broadcasts, user allowlists enforced on both messages and commands. Shared types: `SlackComponentConfigSchema`, `SlackAgentConfigSchema`, `dm.threadPolicy`. Component registry auto-detects per-agent `agent.slack` and loads component even without `components.slack`. Dev mode keeps channels active. README updated with Channels section for Discord + Slack plus manifest setup. 79 test files, 660 tests, all green.
