...
export function createSubagentMcpServer(handlers?: SubagentToolHandlers) {
  ...
  return createSdkMcpServer({
    ...
    tools: [
      ...
      tool(
        SUBAGENT_TOOL_NAMES.status,
        "Get status for a subagent",
        statusSchema,
        async (args) => {
      ...
      tool(
        SUBAGENT_TOOL_NAMES.logs,
        "Fetch logs for a subagent",
        logsSchema,
        async (args) => {
      ...
      tool(
        SUBAGENT_TOOL_NAMES.kill,
        "Kill and remove a subagent workspace",
        killSchema,
        async (args) => {
