...
describe("AgentsView — empty state", () => {
  it("renders empty state when no runs", async () => {
    ...
    expect(container.querySelector("[data-testid='agents-empty']")).not.toBeNull();
  });
});
...
describe("AgentsView — grouping by project", () => {
  it("groups runs by projectId", async () => {
    fetchBoardAgentsMock.mockResolvedValue({
      runs: [
        makeRun({ id: "sar_1", projectId: "PRO-238", sliceId: "PRO-238-S03", label: "Worker" }),
        makeRun({ id: "sar_2", projectId: "PRO-201", sliceId: "PRO-201-S01", label: "Reviewer" }),
        makeRun({ id: "sar_3", projectId: "PRO-238", sliceId: "PRO-238-S04", label: "Worker" }),
      ],
    });
    await new Promise<void>((resolve) => {
      dispose = render(() => <AgentsView />, container);
    ...
    const groupIds = Array.from(groups).map((g) => g.getAttribute("data-testid"));
  ...
  it("groups run without projectId under __unassigned", async () => {
    fetchBoardAgentsMock.mockResolvedValue({
      runs: [
        makeRun({ id: "sar_noproj", label: "Manual" }),
      ],
    });
    await new Promise<void>((resolve) => {
      dispose = render(() => <AgentsView />, container);
...
describe("AgentsView — kill flow", () => {
  it("opens confirmation dialog when kill button clicked", async () => {
    fetchBoardAgentsMock.mockResolvedValue({
      runs: [
        makeRun({ id: "sar_kill_test", projectId: "PRO-7", sliceId: "PRO-7-S01", label: "Worker" }),
      ],
    });
    await new Promise<void>((resolve) => {
      dispose = render(() => <AgentsView />, container);
    ...
    const killBtn = container.querySelector(".agents-btn-kill") as HTMLButtonElement;
  ...
  it("calls killBoardAgent and refreshes on confirm", async () => {
    fetchBoardAgentsMock
      .mockResolvedValueOnce({
        runs: [makeRun({ id: "sar_kill_confirm", projectId: "PRO-8", sliceId: "PRO-8-S01", label: "Worker" })],
      })
      .mockResolvedValue({ runs: [] });
    killBoardAgentMock.mockResolvedValue({ ok: true, runId: "sar_kill_confirm", status: "interrupted" });
    ...
    // Confirm
    const confirmBtn = container.querySelector(".agents-kill-btn-confirm") as HTMLButtonElement;
  ...
  it("cancels dialog without killing when cancel clicked", async () => {
    ...
    const cancelBtn = container.querySelector(".agents-kill-btn-cancel") as HTMLButtonElement;
    cancelBtn.click();
    await Promise.resolve();
