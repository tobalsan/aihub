...
CREATE TABLE IF NOT EXISTS heartbeats (daemon_id TEXT PRIMARY KEY, pid INTEGER, last_tick TEXT, version TEXT);`);
  ...
  listRecent(limit = 50, projectId?: string): unknown[] { return projectId ? this.db.prepare(`SELECT * FROM runs WHERE project_id=? AND (outcome IS NULL OR outcome != 'park_barrier') ORDER BY started_at DESC LIMIT ?`).all(projectId, limit) : this.db.prepare(`SELECT * FROM runs WHERE (outcome IS NULL OR outcome != 'park_barrier') ORDER BY started_at DESC LIMIT ?`).all(limit); }
