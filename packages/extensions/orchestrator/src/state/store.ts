...
CREATE TABLE IF NOT EXISTS heartbeats (daemon_id TEXT PRIMARY KEY, pid INTEGER, last_tick TEXT, version TEXT);`);
  ...
  countRunsByIssue(issueId: string, projectId?: string): number { const row = (projectId ? this.db.prepare(`SELECT COUNT(*) AS count FROM runs WHERE project_id=? AND issue_id=?`).get(projectId, issueId) : this.db.prepare(`SELECT COUNT(*) AS count FROM runs WHERE issue_id=?`).get(issueId)) as { count: number }; return row.count; }
