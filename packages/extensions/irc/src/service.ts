...
export class IrcService {
  ...
  start(): void { this.stopped = false; this.registered = false; this.buffer = ""; this.currentNick = this.config.nick; this.connect(); }
  stop(): void { this.stopped = true; this.registered = false; this.buffer = ""; this.queued = []; if (this.reconnectTimer) clearTimeout(this.reconnectTimer); this.reconnectTimer = null; this.socket?.destroy(); this.socket = null; }
  send(target: string, text: string): void { const line = `PRIVMSG ${target} :${text}`; if (this.registered) this.write(line); else this.enqueue(line); }
  ...
  private connect(): void {
    ...
    const socket: net.Socket | tls.TLSSocket = this.config.tls ? tls.connect({ host: this.config.host, port: this.config.port, servername: this.config.host }, onConnect) : net.connect({ host: this.config.host, port: this.config.port }, onConnect);
    this.socket = socket;
    ...
    socket.on("close", () => { if (this.socket !== socket) return; this.socket = null; this.registered = false; if (!this.stopped) { this.log.warn("[irc] disconnected; reconnecting"); this.reconnectTimer = setTimeout(() => this.connect(), 5_000); } });
  }
  private receive(data: string): void { this.buffer += data; const lines = this.buffer.split("\n"); this.buffer = lines.pop() ?? ""; for (const line of lines) { const message = parseIrcLine(line); if (!message) continue; if (message.command === "PING") this.write(`PONG :${message.trailing ?? message.params[0] ?? ""}`); if (message.command === "001") { this.registered = true; if (this.config.nickservPassword) this.write(`PRIVMSG NickServ :IDENTIFY ${this.config.nickservPassword}`); for (const channel of this.config.channels) this.write(`JOIN ${channel}`); const queued = this.queued.splice(0); queued.forEach((entry) => this.write(entry)); } if (message.command === "433") { this.currentNick = `${this.currentNick}_`; this.write(`NICK ${this.currentNick}`); } this.onMessage(message); } }
  private write(line: string): void { const clean = line.replace(/[\r\n]/g, " "); if (this.socket?.writable) this.socket.write(`${clean}\r\n`); else this.enqueue(clean); }
  private enqueue(line: string): void { const clean = line.replace(/[\r\n]/g, " "); if (this.queued.length < this.maxQueue) this.queued.push(clean); else this.log.warn("[irc] outbound queue full; dropping message"); }
}
