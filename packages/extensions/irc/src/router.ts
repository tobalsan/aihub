...
export class IrcRouter {
  ...
  handle(message: IrcMessage): void {
    ...
    const isChannel = target.startsWith("#") || target.startsWith("&"); const text = normalizeIrcText(message.trailing); const key = isChannel ? target.toLowerCase() : sender.toLowerCase(); this.record(key, sender, text);
    ...
    const route = isChannel ? Object.entries(this.config.channels).find(([channel]) => channel.toLowerCase() === target.toLowerCase())?.[1] : this.config.dm?.enabled === false ? undefined : { agent: this.config.dm?.agent ?? this.config.channels[Object.keys(this.config.channels)[0] ?? ""]?.agent, mode: "reply-all" as const };
  ...
  private run(agentId: string, content: string, destination: string, key: string, isChannel: boolean, sender = destination): void {
    ...
    void this.ctx.runAgent({ agentId, message: content, sessionKey: `irc:${key}`, source: "irc", context }).then(async (result) => { for (const payload of result.payloads) for (const part of splitIrcText(toPlainIrcText(payload.text ?? ""))) { this.service.send(destination, part); await new Promise((resolve) => setTimeout(resolve, 400)); } }).catch((error) => console.error("[irc] agent run failed:", error));
  }
