...
Mention-only channels respond to `aihub: message`; `reply-all` channels answer every message. Channel context is bounded by `historyLimit`. Per-channel A2A cap resets only on configured human nicks.

Agent-local `password` and `nickservPassword` `$env:` references resolve from that agent's `.env` using normal gateway secret resolution.
